...
| `DECAL_ALPHA_BAKED` | info | A decal's opacity mask was composited into its base colour's alpha (RGB dilated under the masked-out texels) and written as `<stem>_decal.tga`. O3DE's decal shader takes opacity from the base colour alpha alone, and the `_decal` filemask selects `Decal_AlbedoWithOpacity` (BC7, alpha kept) instead of `Albedo` (BC1, `DiscardAlpha`). |
| `DECAL_CHANNELS_DROPPED` | info | Roughness/metallic/AO/emissive on a decal material were not exported: the decal pass writes only albedo and normal. |
| `DECAL_MASK_UNMAPPED` | warn | A decal's opacity could not be baked into its base colour alpha (the base colour is not a texture, or the mask is a scalar), so the decal renders its whole projector box. |
...
| `PHYS_SHAPE_DECOMPOSED` | info | UE decomposes this collision into several convex pieces and the staged sidecar (`ue`/`vhacd` mode) asks the cooker to decompose as well, so the cooked product keeps the concavities; one collider references it. |
...
| `DecalActor` | Atom **Decal** component | UE projects along local +X with `decal_size` HALF-extents (x = depth); Atom projects along local −Z over a unit box scaled by entity scale. The importer composes a local Ry(−90) and scale `(2hz, 2hy, 2hx)` (decal_build, matrix-identity tested). The material converts through StandardPBR, not an Atom decal material type → `DECAL_MATERIAL_APPROX`; `Sort Key` maps from `sort_order`. A decal material is exported as its OWN variant (`<path>#decal`), because Atom's decal pass reads only albedo and normal and takes opacity from the ALBEDO'S ALPHA (`Decals.azsli`: `baseMap.a * m_opacity * attenuation`) — no opacity map exists in the shader or in the feature processor's texture arrays. The UE opacity mask is composited into that alpha → `DECAL_ALPHA_BAKED` |
