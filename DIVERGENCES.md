...
| LODs | per-distance LOD chain + screen-size switching; Nanite renders the full source at every distance | **a real chain on the FBX path** (one `.azmodel`, N `.azlod`). Nanite meshes export by default as their fallback mesh plus UE's render LODs (6–9 % of the source); `UEO3DE_NANITE_FALLBACK=0` exports the Nanite source simplified through UE's reducer at `UEO3DE_LOD_RATIOS` / `UEO3DE_LOD0_RATIO` budgets. Switch distances are O3DE's defaults — UE's per-LOD screen sizes are not authored yet. glb exports stay flattened (`LOD_FLATTENED`). See LODS_AND_COLLISION.md |
