...
| Convex decomposition | a mesh's simple collision is N convex hulls, authored or generated | **UE's real hulls are unreachable.** `KConvexElem` exposes no vertex accessor from Python (measured: `dir()` yields only generic struct methods, and `get_editor_property('vertex_data'/'vertices')` fails on real elements), so the manifest carries each hull's count and AABB and nothing else. The importer therefore cooks ONE hull of the whole render mesh and every concavity UE decomposed away is filled back in — 1,066 times on one converted level, reported as `PHYS_SHAPE_APPROXIMATED`. `UEO3DE_DECOMPOSE=1` turns on cook-time V-HACD, which approximates the decomposition instead; off by default because it is minutes-per-mesh on dense geometry | identical, and identically limited. No non-licensed fixture contains a multi-convex asset, so this path has unit coverage only |
