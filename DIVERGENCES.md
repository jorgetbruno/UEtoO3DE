...
| Physical materials | a `UPhysicalMaterial` per material (friction, restitution, density, and combine modes) | **not carried, at any layer.** The exporter has no notion of physical materials, so the manifest holds only render material names; the importer writes no `PhysicsMaterialSlots` into the physics mesh group; every collider gets the backend's default surface. Measured before assuming it mattered (`Tests/ue/probe_phys_materials.py`): the fixture project contains **0** `PhysicalMaterial` assets and 0 of 8 mesh slots reference one, so nothing is lost on current content — but real content that uses them would silently get default friction with no warning. Note the licensed packs are not in that project, so this says nothing about them | identical, and the gap is one layer deeper than it looks: Jolt's cooked trimesh blob (v2) already stores a material-slot index per face and resolves it live at contact time, so the backend is ready and the data never arrives. UE's `FrictionCombineMode`/`RestitutionCombineMode` have no O3DE equivalent and would be a divergence in their own right |
...
| Non-uniform scale on colliders | collider shapes inherit the actor's full 3-axis scale | **faithful.** The engine applies the entity's scale to its own colliders, in entity space, so the importer authors the manifest's own numbers and multiplies nothing — dimensions, offsets, primitives and cooked assets alike. Nothing is approximated, so no `PHYS_SHAPE_APPROXIMATED` for scale. (Only when a backend does *not* advertise `CAP_SCALE_ENGINE_APPLIED`, i.e. under `UEO3DE_BAKE_SCALE=1` for an older gem, does the importer bake it — and then a sphere under non-uniform scale takes the largest axis and is reported) | same, measured identical on both: every ratio 2.000, and a rotated collider composes in entity space. See the section below |
| Convex decomposition | a mesh's simple collision is N convex hulls, authored or generated | **three representations, `UEO3DE_COLLISION` at staging.** `single` (default): ONE hull of the whole render mesh, every concavity filled in → `PHYS_SHAPE_APPROXIMATED`. `vhacd`: cook-time V-HACD capped at UE's element count, from LOD 1 (minutes per dense mesh otherwise). `ue`: UE's own elements — `KConvexElem` fields are protected from Python, but the whole `AggGeom` struct copies onto the baked asset and UE's FBX writer emits ONE merged `UCX_<node>_LOD0` node (measured, 132 vertices for 10 elements); the physics group selects it, decomposes it capped at the element count (disjoint elements come back exactly, overlapping ones merge: tow truck 10/10, pickup 9/10, box truck 5/10) and carries a half-turn `CoordinateSystemRule` because the hulls are source-space while the render mesh is baked. `#mx` mirrored variants stay `single` — a reflection is not a rotation | same sidecar per backend; one shape per selected node on a convex group in both gems (measured on Jolt, structural on PhysX: `MergeMeshes` lives in `TriangleMeshAssetParams` in both). No non-licensed fixture contains a multi-convex asset, so unit coverage pins the sidecar and the RetroCars fleet is the live check |
...
| Landscape holes | hole material carves visibility + collision | holes are sampled as their neighbours (C0 fill); neither the visual nor the collision hole survives. **Measured, and it is not level with the surrounding surface**: on L_Showcase a sphere dropped over a filled hole rests ~0.22 m BELOW where the same drop rests on ordinary terrain, so the fill sits under the surface rather than flush with it. `Tests/m7`'s drop test flags exactly that (its tolerance is 0.070 m) — the failure is this divergence being real, not a regression, and the fill offset is worth measuring properly before anyone walks a character over one |
...
| LODs | per-distance LOD chain + screen-size switching; Nanite renders the full source at every distance | **a real chain on the FBX path** (one `.azmodel`, N `.azlod`). Nanite meshes export as the Nanite source simplified through UE's reducer (`UEO3DE_LOD0_RATIO` 0.25 at LOD 0, `UEO3DE_LOD_RATIOS` 10/4/1.5/0.6 % beneath); UE's fallback mesh is opt-in (`UEO3DE_NANITE_FALLBACK=1`) because its LOD 0 material sections are measured wrong on permuted-section meshes. Switch distances are O3DE's defaults — UE's per-LOD screen sizes are not authored yet. glb exports stay flattened (`LOD_FLATTENED`). See LODS_AND_COLLISION.md |
